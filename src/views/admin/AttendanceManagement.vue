<template>
  <v-container>
    <h1 class="text-h4 mb-4">Gestión de Asistencias</h1>
    <v-card class="pa-4">
...
export default {
  ...
  created() {
    ...
    this.fetchRelatedData(); // Cargar clases, usuarios, profesores, membresías
  },
  ...
  methods: {
    ...
    async fetchRelatedData() {
      try {
        const [clasesRes, usersRes, membershipsRes] = await Promise.all([
        ...
        this.professors = usersRes.data.filter(user => user.rol === 'profesor'); // Filtrar solo profesores
